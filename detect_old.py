...
def _read_video_metadata(source: str) -> dict[str, float]:
    if source.isdigit() or shutil.which("ffprobe") is None:
        return {}
...
def _scale_detection(det: Detection | None, scale: float, x_offset: int, y_offset: int) -> Detection | None:
    ...
    return Detection(
        ...
        conf=det.conf,
        xyxy=_scale_box(det.xyxy, scale, x_offset, y_offset),
    )


def _scale_detections(detections: list[Detection], scale: float, x_offset: int, y_offset: int) -> list[Detection]:
    scaled = []
...
def _scale_statuses(statuses: list[PersonPPEStatus], scale: float, x_offset: int, y_offset: int) -> list[PersonPPEStatus]:
    scaled_statuses = []
